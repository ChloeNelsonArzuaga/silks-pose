...
Usage:
    python run.py input_video.mov output_prefix
...
def main(video_path, output_prefix):
    ...
    preprocessed_dir = Path("data/preprocessed") / output_prefix.name
    preprocessed_dir.mkdir(parents=True, exist_ok=True)

    silk_dir = Path("data/preprocessed_silk") / output_prefix.name
    silk_dir.mkdir(parents=True, exist_ok=True)

    output_dir = Path("data/output") / output_prefix.name
    output_dir.mkdir(parents=True, exist_ok=True)

    # Step 1: Extract landmarks
    run_command(
        ["python3", "pipeline/extract_landmarks.py", str(video_path), str(landmarks_json)],
        "Extract pose landmarks from video"
    )

    # Step 2: Preprocess landmarks
    run_command(
        ["python3", "pipeline/preprocess.py", str(landmarks_json), str(video_path), str(preprocessed_dir)],
    ...
    print(f"{'='*60}")
    print(f"\nOutputs generated in: data/output/{output_prefix.name}/")
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        # Default: run on test video when launched from play button
        main("data/raw_videos/test.mov", "test")
    else:
